...
export class TontinesService {
  ...
  async getGroupDetail(groupId: string, userId: string) {
    ...
    const { data: members } = await this.db
      .from('tontine_members')
      .select('id, user_id, join_order, payout_position, status')
      .eq('group_id', groupId)
  ...
  async contribute(groupId: string, callerId: string, pin: string, idempotencyKey: string) {
    ...
    await this.db
      .from('tontine_contributions')
      .update({ status: 'paid', transfer_id: result.transfer.id, paid_at: new Date().toISOString(), updated_at: new Date().toISOString() })
    ...
    if ((pendingCount || 0) === 0) {
      ...
      if (cycle.cycle_number >= group.max_members) {
        groupCompleted = true;
        await this.db.from('tontine_groups').update({ status: 'completed', updated_at: new Date().toISOString() }).eq('id', groupId);
      } else {
        const { data: allMembers } = await this.db.from('tontine_members').select('id').eq('group_id', groupId).eq('status', 'active');
        const { data: nextRecipient } = await this.db
          ...
          .single();
        if (!nextRecipient) {
          throw new Error(`Tontine data inconsistency: no member at payout_position ${cycle.cycle_number + 1} for group ${groupId}`);
        }

        const nextCycle = await this.createCycle(groupId, cycle.cycle_number + 1, nextRecipient.id, group.frequency);
        await this.generateContributions(nextCycle.id, groupId, nextRecipient.id, allMembers || [], group.contribution_amount_cents, group.currency, nextCycle.due_date);
        await this.db.from('tontine_groups').update({ current_cycle: cycle.cycle_number + 1, updated_at: new Date().toISOString() }).eq('id', groupId);
      }
    }

    return { contribution: { ...contribution, status: 'paid' }, transfer: result.transfer, cycle_completed: cycleCompleted, group_completed: groupCompleted };
  }
}
